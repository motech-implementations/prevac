...
public class CustomColumnWidthPdfTableWriter extends PdfTableWriter {
    ...
    private float[] calculateColumnWidths(String[] headers) throws DocumentException {
        ...
        for (int i = 0; i < dataTable.getNumberOfColumns(); i++) {
            if (PrevacConstants.REPORT_COLUMN_WIDTHS.containsKey(headers[i])) {
                spaceForTheRestOfColumns -= PrevacConstants.REPORT_COLUMN_WIDTHS.get(headers[i]);
        ...
        for (int i = 0; i < dataTable.getNumberOfColumns(); i++) {
            if (PrevacConstants.REPORT_COLUMN_WIDTHS.containsKey(headers[i])) {
                allWidths[i] = PrevacConstants.REPORT_COLUMN_WIDTHS.get(headers[i]);
            } else {
